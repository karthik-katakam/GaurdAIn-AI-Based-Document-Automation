...
//Show progress bar percentage (and increasing) if percentage is <100. Once 100+, state that its complete.
  ...
  const handleSubmitForScan = () => {
    //get the recaptcha result(true/false = pass/fail)
    const captchaValue = recaptcha.current.getValue();
    // alert("token: "+ captchaValue)
    if (selectedFile) {
      const fileUrl = URL.createObjectURL(selectedFile);
      if(captchaValue)
      {
        //if catpcha has been done, send input to google backend api and verify token(need to run backend server)
          fetch(`http://localhost:${port}/verify-captcha`, {
          method: 'POST',
        ...
        .then((resp) => resp.json())
        .then((data)=>{
          //get the verification result. if its positive, let us navigate to file analysis page
          if(data && data.success)
          {
            navigate("/file-analysis", { state: { fileUrl } }); // Pass file URL correctly
            console.log(data)
          ...
          {
            //if recaptcha verification negative, alert user
            alert("reCAPTCHA failed!" + data);

          }

        })
      }
      //alert if reCaptcha is not done
      else{
